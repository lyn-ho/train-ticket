...
export function createAdult() {
  return (dispatch, getState) => {
    ...
    dispatch(
      setPassengers([
        ...passengers,
        { id: ++passengerIdSeed, name: '', ticketType: 'adult', licenceNo: '', seat: 'Z' },
      ]),
    )
  }
}
    ...
    dispatch(
      showMenu({
        ...
        options: passengers
          ...
          .map((adult) => ({ title: adult.name, value: adult.id, active: adult.id === passenger.followAdult })),
      }),
    )
  }
}
  ...
  return (dispatch, getState) => {
    ...
    dispatch(
      showMenu({
        onPress(ticketType) {
          if ('adult' === ticketType) {
            dispatch(updatePassenger(id, { licenceNo: '', ticketType }, ['gender', 'followAdult', 'birthday']))
          } else {
            const adult = passengers.find((passenger) => passenger.id !== id && passenger.ticketType === 'adult')

            if (adult) {
              dispatch(
                updatePassenger(id, { gender: '', followAdult: adult.id, birthday: '', ticketType }, ['licenceNo']),
              )
            } else {
              alert('没有其他成人乘客')
            }
        ...
        options: [
          { title: '成人票', value: 'adult', active: 'adult' === passenger.ticketType },
          { title: '儿童票', value: 'child', active: 'child' === passenger.ticketType },
        ],
      }),
    )
  }
